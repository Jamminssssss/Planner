...
enum CSVMode: String, CaseIterable {
    ...
    var icon: String {
        switch self {
        case .allPlans:  return "list.bullet.rectangle"
        case .workMonth: return "hammer.fill"
        }
    }
    var description: String {
        switch self {
        case .allPlans:  return "Export plans with full details"
        case .workMonth: return "Monthly work schedule & income"
        }
    }
}

// MARK: - Export Hub View (PDF + CSV)
...
struct PDFExportView: View {
    ...
    private var workMonthPlans: [Plan] {
        let cal = Calendar.current
        let y = cal.component(.year,  from: workMonth)
        let m = cal.component(.month, from: workMonth)
        ...
        NavigationStack {
            ...
            .sheet(isPresented: $showPDFShare) {
                if let pdf = generatedPDF {
                    PDFShareSheet(pdfData: pdf, fileName: pdfFileName())
                }
            }
            .sheet(isPresented: $showCSVShare) {
                if let csv = generatedCSV {
                    CSVShareSheet(csvData: csv, fileName: csvFileName())
                }
            }
        }
    }
        ...
        HStack(spacing: 0) {
            ForEach(ExportTab.allCases, id: \.self) { tab in
                Button(action: {
                    ...
                    .background(
                        selectedTab == tab
                            ? (tab == .pdf ? Color.green : Color.blue)
                            : Color.clear
                    )
                    .cornerRadius(10)
                }
    ...
    private var pdfContent: some View {
        VStack(spacing: 20) {
            headerCard(
                icon: "doc.richtext.fill", iconColor: .green,
                title: "PDF Report",
                subtitle: "Formatted report with cover page & statistics",
                count: previewCount, countColor: .green
            )
            rangeSection
            contentSection
            actionButton(
                label: isGeneratingPDF ? "Generating..." : (previewCount == 0 ? "No Plans" : "Generate & Share PDF"),
                icon: "square.and.arrow.up", color: .green,
                isLoading: isGeneratingPDF,
                disabled: isGeneratingPDF || previewCount == 0,
                action: generatePDF
            )
        }
    }
    ...
    private var csvContent: some View {
        VStack(spacing: 20) {
            headerCard(
                icon: "tablecells.fill", iconColor: .blue,
                title: "CSV Spreadsheet",
                subtitle: "Open in Google Sheets, Excel, or Numbers",
                count: csvMode == .workMonth ? workMonthPlans.count : previewCount,
                countColor: .blue
            )
            sheetsGuideCard
            csvModeSection
            ...
            actionButton(
                label: isGeneratingCSV ? "Generating..." : "Export CSV for Google Sheets",
                icon: "arrow.up.doc.fill", color: .blue,
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text("\(count) plan\(count == 1 ? "" : "s")")
                        .font(.system(size: 12, weight: .medium)).foregroundColor(countColor)
                }
                .padding(.horizontal, 10).padding(.vertical, 4)
                .background(countColor.opacity(0.10)).cornerRadius(8)
                .padding(.top, 2)
            }
            Spacer()
        }
    ...
    private var sheetsGuideCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                ...
                Text("How to open in Google Sheets")
                    .font(.system(size: 13, weight: .semibold)).foregroundColor(.blue)
            }
            VStack(alignment: .leading, spacing: 6) {
                guideStep("1", "Tap [Export CSV] → tap Share")
                guideStep("2", "Save to Files or send via email/AirDrop")
                guideStep("3", "Google Sheets → New → Import → Upload .csv")
                guideStep("4", "Select [Replace spreadsheet] → Import data ✅")
            }
        }
        .padding(14)
    ...
    private func guideStep(_ num: String, _ text: String) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Text(num)
                ...
                .frame(width: 18, height: 18).background(Color.blue).cornerRadius(9)
                    ...
                    Button(action: { csvMode = mode }) {
                        VStack(spacing: 6) {
                            Image(systemName: mode.icon)
                                .font(.system(size: 20))
                                .foregroundColor(csvMode == mode ? .white : .blue)
                            Text(mode.rawValue)
                                .font(.system(size: 12, weight: .semibold))
                                .foregroundColor(csvMode == mode ? .white : .primary)
                            Text(mode.description)
                                .font(.system(size: 10))
                                .foregroundColor(csvMode == mode ? .white.opacity(0.85) : .secondary)
                                .multilineTextAlignment(.center)
                        }
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 14).padding(.horizontal, 8)
                        .background(csvMode == mode ? Color.blue : Color.secondary.opacity(0.07))
                        .cornerRadius(12)
                        .overlay(
                            RoundedRectangle(cornerRadius: 12)
                                .stroke(csvMode == mode ? Color.blue : Color.clear, lineWidth: 2)
                        )
                    }
                    .buttonStyle(.plain)
                }
    ...
    private var workMonthSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                Button(action: {
                    workMonth = Calendar.current.date(byAdding: .month, value: -1, to: workMonth) ?? workMonth
                }) {
                    Image(systemName: "chevron.left")
                        .font(.system(size: 16, weight: .semibold)).foregroundColor(.blue)
                        .frame(width: 36, height: 36)
                        .background(Color.blue.opacity(0.10)).cornerRadius(10)
                }
                .buttonStyle(.plain)
            ...
            if wPlans.isEmpty {
                Text("No work schedules for this month.")
                    ...
                    Button(action: { options.range = range }) {
                        HStack(spacing: 14) {
                            Image(systemName: range.systemImage)
                                .font(.system(size: 17))
                                .foregroundColor(options.range == range ? accent : .secondary)
                                .frame(width: 24)
                        ...
                        .background(options.range == range ? accent.opacity(0.08) : Color.secondary.opacity(0.05))
                        .cornerRadius(10)
                        .overlay(
                            RoundedRectangle(cornerRadius: 10)
                                .stroke(options.range == range ? accent.opacity(0.4) : Color.clear, lineWidth: 1.5)
                        )
                    }
                    .buttonStyle(.plain)
                }
    ...
    private var contentSection: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(spacing: 0) {
                toggleRow(icon: "checkmark.circle.fill", iconColor: .green,
                          label: "Completed Plans",   isOn: $options.includeCompleted, accent: accent)
                Divider().padding(.leading, 52)
                toggleRow(icon: "clock.fill", iconColor: .blue,
                          label: "Planned (Upcoming)", isOn: $options.includePlanned, accent: accent)
                Divider().padding(.leading, 52)
                toggleRow(icon: "xmark.circle.fill", iconColor: .red,
                          label: "Canceled Plans",    isOn: $options.includeCanceled, accent: accent)
                Divider().padding(.leading, 52)
                toggleRow(icon: "hammer.fill", iconColor: .orange,
                          label: "Work Schedule",     isOn: $options.includeWorkSchedule, accent: accent)
                Divider().padding(.leading, 52)
                toggleRow(icon: "text.alignleft", iconColor: .gray,
                          label: "Memo / Notes",      isOn: $options.includeMemo, accent: accent)
                if selectedTab == .pdf {
                    Divider().padding(.leading, 52)
                    toggleRow(icon: "chart.bar.fill", iconColor: .purple,
                              label: "Statistics Summary", isOn: $options.includeStats, accent: accent)
                }
            }
        }
        .padding(16)
    ...
    private func toggleRow(icon: String, iconColor: Color, label: String,
                           isOn: Binding<Bool>, accent: Color) -> some View {
        HStack(spacing: 14) {
            Image(systemName: icon).font(.system(size: 17)).foregroundColor(iconColor).frame(width: 24)
            Text(label).font(.system(size: 15)).foregroundColor(.primary)
            Spacer()
            Toggle("", isOn: isOn).tint(accent).labelsHidden()
        }
        .padding(.vertical, 12)
    }
    ...
    private func actionButton(
        ...
        isLoading: Bool, disabled: Bool, action: @escaping () -> Void
    ) -> some View {
    ...
    private var workMonthTitle: String {
        let f = DateFormatter(); f.dateFormat = "MMMM yyyy"; f.locale = Locale(identifier: "en_US")
        return f.string(from: workMonth)
    }

    private func pdfFileName() -> String {
        let f = DateFormatter(); f.dateFormat = "yyyy-MM-dd"
        return "GrassPlanner_\(options.range.rawValue.replacingOccurrences(of: " ", with: "_"))_\(f.string(from: Date())).pdf"
    }
    ...
    private func csvFileName() -> String {
        ...
        if csvMode == .workMonth {
            ...
            return "GrassPlanner_Work_\(mf.string(from: workMonth)).csv"
        }
    ...
    private func generatePDF() {
        ...
        generatedPDF    = PDFExportService.shared.generatePDF(plans: allPlans, options: options)
    ...
    private func generateCSV() {
        ...
        if csvMode == .workMonth {
            ...
            csvString = CSVExportService.shared.generateCSV(plans: allPlans, options: options)
        }
        generatedCSV    = csvString.data(using: .utf8)
